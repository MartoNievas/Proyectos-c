 ...
 */
static int run_fips_test_case(const char *test_name, const uint8_t *key,
                              aes_key_size_t key_size, const uint8_t *plaintext,
  ...
  if (memcmp(plaintext, decrypted_plaintext, AES_BLOCK_SIZE) != 0) {
    fprintf(stderr,
            "FAIL: Decrypted text does not match the original plaintext.\n");
    return 1;
  }
...
int main(void) {
  ...
  const uint8_t plaintext128[] = {0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a,
  ...
  const uint8_t ciphertext128[] = {0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc,
                                   ...
                                   0x19, 0x6a, 0x0b, 0x32};
  failed_tests +=
      run_fips_test_case("AES-128 FIPS-197 C.1", key128, AES_KEY_SIZE_128,
                         plaintext128, ciphertext128);

  // Test Case 2: AES-192 from FIPS-197 Appendix C.2
  const uint8_t key192[] = {0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52,
                            0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5,
                            0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b};
  const uint8_t plaintext192[] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40,
                                  0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11,
                                  0x73, 0x93, 0x17, 0x2a};
  const uint8_t ciphertext192[] = {0xbd, 0x33, 0x4f, 0x1d, 0x6e, 0x45,
                                   0xf2, 0x5f, 0xf7, 0x12, 0xa2, 0x14,
                                   0x57, 0x1f, 0xa5, 0xcc};
  failed_tests +=
      run_fips_test_case("AES-192 FIPS-197 C.2", key192, AES_KEY_SIZE_192,
                         plaintext192, ciphertext192);
  ...
  const uint8_t plaintext256[] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40,
                                  0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11,
                                  0x73, 0x93, 0x17, 0x2a};
  const uint8_t ciphertext256[] = {0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2,
                                   0xa0, 0x3c, 0x06, 0x4b, 0x5a, 0x7e,
                                   0x3d, 0xb1, 0x81, 0xf8};
  failed_tests +=
      run_fips_test_case("AES-256 FIPS-197 C.3", key256, AES_KEY_SIZE_256,
                         plaintext256, ciphertext256);
